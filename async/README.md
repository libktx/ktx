...
*Loading same asset multiple times* | **Supported**. `load` and `unload` can be called with the same asset path multiple times on multiple coroutines at the same time. Coroutines are suspended until the asset is loaded if another thread is already handling the loading of requested asset. Thanks to references counting, assets are not fully disposed of until `unload` is called the same amount of times as `load`. | **Not supported.** Trying to load the same asset multiple times causes a runtime exception.
