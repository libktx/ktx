...
fun horizontalFlowGroup(spacing: Float = 0f, init: KHorizontalFlowGroup.() -> Unit): HorizontalFlowGroup
...
fun verticalGroup(init: KVerticalGroup.() -> Unit): VerticalGroup = actor(KVerticalGroup(), init)

/** Begins creation of UI using type-safe builder, uses [VerticalFlowGroup] as base widget container. */
fun verticalFlowGroup(spacing: Float = 0f, init: KVerticalFlowGroup.() -> Unit): VerticalFlowGroup
...
fun gridGroup(itemSize: Float = 256f, spacing: Float = 8f, init: KGridGroup.() -> Unit): GridGroup
...
fun floatingGroup(init: KFloatingGroup.() -> Unit): FloatingGroup = actor(KFloatingGroup(), init)

/** Begins creation of UI using type-safe builder, uses [FloatingGroup] as base widget container. */
fun floatingGroup(prefWidth: Float, prefHeight: Float, init: KFloatingGroup.() -> Unit): FloatingGroup
...
fun stack(init: KStack.() -> Unit): Stack = actor(KStack(), init)

internal fun <T : Actor> actor(actor: T, init: T.() -> Unit): T {
  actor.init()
  return actor
}
